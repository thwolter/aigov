...
use crate::properties::metadata::{HeadingPair, Metadata};
use crate::xml::{TextElement, parse_text_elements, write_text_element};
use quick_xml::{
    Reader, Writer,
...
fn apply_properties(properties: Vec<TextElement>, metadata: &mut Metadata) {
...
fn read_vectors(xml: &[u8], metadata: &mut Metadata) -> Result<()> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            {
                if values.len() % 2 != 0 {
                    return Err(OoxmlError::InvalidDocument(
...
fn invalid_vector(error: impl std::fmt::Display) -> OoxmlError {
    OoxmlError::InvalidDocument(format!("Invalid extended metadata: {error}"))
}
...
fn create_extended_properties(metadata: &Metadata) -> Result<Vec<u8>> {
...
mod tests {
    use super::*;
    use crate::properties::metadata::{ExtendedMetadata, HeadingPair, Metadata};
