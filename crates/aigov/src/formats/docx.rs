...
impl DocxDocument {
    ...
    pub fn document_xml(&self) -> Result<&[u8]> {
        Ok(self.package.read_part(&DOCUMENT_XML.into())?)
    }
...
impl OfficeDocument for DocxDocument {
    ...
    fn metadata(&self) -> Result<Metadata> {
        Ok(ooxml::read_metadata(&self.package)?)
    }

    fn set_metadata(&mut self, metadata: &Metadata) -> Result<()> {
        Ok(ooxml::write_metadata(&mut self.package, metadata)?)
    }
    ...
    fn save(&self, destination: &Path) -> Result<()> {
        Ok(self.package.save(destination)?)
    }
}

impl AiPolicyDocument for DocxDocument {
    fn has_policy(&self) -> Result<bool> {
        policy::has_policy_marker(self.package.read_part(&DOCUMENT_XML.into())?)
    }

    fn inject_policy(&mut self, policy: &AiPolicy) -> Result<()> {
        if self.has_policy()? {
            return Err(OfficeError::InvalidAiPolicy(
    ...
    fn update_policy(&mut self, policy: &AiPolicy) -> Result<()> {
        let part = PartName::new(DOCUMENT_XML);
        let (updated, xml) =
    ...
    fn remove_policy(&mut self) -> Result<bool> {
        let part = PartName::new(DOCUMENT_XML);
