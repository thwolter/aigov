...
pub fn parse_text_elements(xml: &[u8]) -> Result<Vec<TextElement>> {
...
pub fn write_text_element(writer: &mut Writer<Vec<u8>>, name: &str, value: &str) -> Result<()> {
...
fn invalid_property_text(error: impl std::fmt::Display) -> OoxmlError {
    OoxmlError::InvalidDocument(format!("Invalid metadata property text: {error}"))
}
