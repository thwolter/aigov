...
pub(in crate::ooxml) fn insert_before_closing_tag(
    xml: &[u8],
    closing_tag: &str,
    addition: &str,
    already_present: &str,
    ...
    let xml = std::str::from_utf8(xml).map_err(|error| {
        crate::OfficeError::InvalidDocument(format!("Invalid package XML: {error}"))
    })?;
    ...
    let Some(index) = xml.rfind("/>") else {
        return Err(crate::OfficeError::InvalidDocument(format!(
